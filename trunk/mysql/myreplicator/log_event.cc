...
void Query_log_event::bprint(uchar *buf, PRINT_EVENT_INFO* print_event_info)
{
  Write_on_release_cache cache(&print_event_info->head_cache, result_file);

  print_query_header(&cache, print_event_info);
  my_b_write(&cache, (uchar*) query, q_len);
  my_b_printf(&cache, "\n%s", print_event_info->delimiter);
}
#endif /* MYSQL_CLIENT */
