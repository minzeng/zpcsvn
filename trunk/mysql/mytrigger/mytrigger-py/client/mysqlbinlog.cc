...
#ifndef DBUG_OFF
static const char* default_dbug_option = "d:t:o,/tmp/mytrigger.trace";
#endif
...
static void sigtermHandler(int sig) {
	...
	if (SIGTERM == sig) {
		MYLOG(ZPC_NOTICE, "SIGTERM received, scheduling shutting down...");
	} else if (SIGINT == sig){
		MYLOG(ZPC_NOTICE, "SIGINT received, scheduling shutting down...");
	} else {
		return;
	}
	shutdown_flag = 1;
							 ...
							 char *filename)
{
  ...
  if ((file= create_unique_file(filename,tail)) < 0)
  {
    ...
    MYLOG(ZPC_ERROR, "Could not construct local filename %s.",filename);
    return -1;
  }
                                                     ...
                                                     File file)
{
  ...
  if (my_net_write(net, buf, server_fname_len +2) || net_flush(net))
  {
    ...
    MYLOG(ZPC_ERROR, "Failed requesting the remote dump of %s.", server_fname);
    return ERROR_STOP;
  }
  ...
  for (;;)
  {
    ...
    if (packet_len == 0)
    {
      if (my_net_write(net, (uchar*) "", 0) || net_flush(net))
      {
        ...
        MYLOG(ZPC_ERROR, "Failed sending the ack packet.");
        return ERROR_STOP;
      }
      /*
    ...
    else if (packet_len == packet_error)
    {
      ...
      MYLOG(ZPC_ERROR, "Failed reading a packet during the dump of %s.", server_fname);
      return ERROR_STOP;
    }
    ...
    if (packet_len > UINT_MAX)
    {
      ...
      MYLOG(ZPC_ERROR, "Illegal length of packet read from net.");
      return ERROR_STOP;
    }
                                                    ...
                                                    Create_file_log_event *ce)
{
  ...
  if ((file= create_unique_file(fname,ptr)) < 0)
  {
    ...
    MYLOG(ZPC_ERROR, "Could not construct local filename %s%s.",
          target_dir_name,bname);
    delete ce;
    DBUG_RETURN(ERROR_STOP);
  }
  ...
  if (my_write(file, (uchar*)block, block_len, MYF(MY_WME|MY_NABP)))
  {
    ...
    MYLOG(ZPC_ERROR, "Failed writing to file.");
    retval= ERROR_STOP;
  }
  if (my_close(file, MYF(MY_WME)))
  {
    error("Failed closing file.");
    MYLOG(ZPC_ERROR, "Failed closing file.");
    retval= ERROR_STOP;
  }
  DBUG_RETURN(retval);
}
...
Exit_status Load_log_processor::process(Append_block_log_event *ae)
{
  ...
  if (fname)
  {
			...
			O_APPEND|O_BINARY|O_WRONLY,MYF(MY_WME))) < 0))
    {
      ...
      MYLOG(ZPC_ERROR, "Failed opening file %s", fname);
      DBUG_RETURN(ERROR_STOP);
    }
    if (my_write(file,(uchar*)ae->block,ae->block_len,MYF(MY_WME|MY_NABP)))
    {
      error("Failed writing to file %s", fname);
      MYLOG(ZPC_ERROR, "Failed writing to file %s", fname);
      retval= ERROR_STOP;
    }
    if (my_close(file,MYF(MY_WME)))
    {
      error("Failed closing file %s", fname);
      MYLOG(ZPC_ERROR, "Failed closing file %s", fname);
      retval= ERROR_STOP;
    }
    DBUG_RETURN(retval);
  }
                              ...
                              PRINT_EVENT_INFO *print_event_info)
{
      ...
      copy_event_cache_to_file_and_reinit(body, result_file))
  {
    ...
    MYLOG(ZPC_ERROR, "Error writing event to file.");
    DBUG_RETURN(ERROR_STOP);
  }
  DBUG_RETURN(OK_CONTINUE);
}
    ...
    case FORMAT_DESCRIPTION_EVENT:
      ...
      */
          ...
          (glob_description_event->flags & LOG_EVENT_BINLOG_IN_USE_F))
      {
        ...
        MYLOG(ZPC_ERROR, "Attempting to dump binlog '%s', which was not closed properly. "
              "Most probably, mysqld is still writing it, or it crashed. "
              "Rerun with --force-if-open to ignore this problem.", logname);
        DBUG_RETURN(ERROR_STOP);
      }
      ...
      if (!print_event_info->printed_fd_event && !short_form)
      {
        ...
        if (opt_base64_output_mode == BASE64_OUTPUT_NEVER) {
          error("--base64-output=never specified, but binlog contains a "
                ...
                type_str);
          MYLOG(ZPC_ERROR, "--base64-output=never specified, but binlog contains a "
                "%s event which must be printed in base64.",
                type_str);
		} else {
          error("malformed binlog: it does not contain any "
                ...
                type_str);
          MYLOG(ZPC_ERROR, "malformed binlog: it does not contain any "
                "Format_description_log_event. I now found a %s event, which "
                "is not safe to process without a "
                "Format_description_log_event.",
                type_str);
...
static struct my_option my_long_options[] =
{
  ...
  */
  ...
  {"self-server-id", 904,
   ...
   &server.self_server_id, &server.self_server_id, 0, GET_ULONG,
   REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"mytrigger-info", 905, "mytrigger conf", &server.master_info_file,
	  &server.master_info_file,
   0, GET_STR_ALLOC, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"daemon", 906, "run as daemon.",
   &server.is_daemonize, &server.is_daemonize, 0, GET_BOOL, NO_ARG, 0, 0, 0, 0,
   0, 0},
  {"log-file", 907, "mytrigger log file", &server.log_file, &server.log_file,
   0, GET_STR_ALLOC, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"mytrigger-so", 908, "mytrigger so file", &server.mytrigger_so_file,
	  &server.mytrigger_so_file,
   0, GET_STR_ALLOC, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"skip-slave-error", 909, "Skip slave sql error",
   &server.skip_slave_error, &server.skip_slave_error, 0, GET_BOOL,
   NO_ARG, 0, 0, 0, 0, 0, 0},
  {"python-script", 912, "so configure file",
  &server.python_script, &server.python_script,
   0, GET_STR_ALLOC, REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"row-queue-size", 913,
   "The row queue size.",
   &server.row_queue_size, &server.row_queue_size, 0, GET_ULONG,
   REQUIRED_ARG, 10000, 0, 0, 0, 0, 0},
  {"log-level", 914, "log file level",
...
static my_time_t convert_str_to_timestamp(const char* str)
{
      ...
      MYSQL_TIMESTAMP_DATETIME || was_cut)
  {
    ...
    MYLOG(ZPC_ERROR, "Incorrect date and time argument: %s", str);
    exit(1);
  }
  /*
...
static Exit_status safe_connect()
{
  ...
  if (!mysql_real_connect(mysql, host, user, pass, 0, port, sock, 0))
  {
	...
	MYLOG(ZPC_ERROR, "connect to master.[%s:%d] user: %s", host, port, user);
    MYLOG(ZPC_ERROR, "Failed on connect to master: %s", mysql_error(mysql));
    return ERROR_STOP;
  }
  mysql->reconnect= 1;
  return OK_CONTINUE;
  ...
  */
  ...
  print_event_info.verbose= short_form ? 0 : verbose;
	...
	for (;;) {
		...
		MYLOG(ZPC_ERROR, 
			"dump_remote_log_entries returned[%d], scheduling shutting down...",
			rc);
...
static Exit_status check_master_version()
{
      ...
      !(res = mysql_store_result(mysql)))
  {
    ...
    MYLOG(ZPC_ERROR, "Could not find server version: "
          "Query failed when checking master version: %s", mysql_error(mysql));
    return ERROR_STOP;
  }
...

{
  ...
  */
  if ((retval= safe_connect()) != OK_CONTINUE) {
	MYLOG(ZPC_ERROR, "safe connect faild!");
    DBUG_RETURN(retval);
  }
  net= &mysql->net;

  if ((retval= check_master_version()) != OK_CONTINUE) {
	MYLOG(ZPC_ERROR, "check master version faild!");
    DBUG_RETURN(retval);
  }
  ...
  */
  ...
  if (tlen > UINT_MAX) 
  {
    ...
    MYLOG(ZPC_ERROR, "Log name too long.");
    DBUG_RETURN(ERROR_STOP);
  }
  logname_len = (uint) tlen;
  int4store(buf + 6, server.self_server_id);
  memcpy(buf + 10, logname, logname_len);
  if (simple_command(mysql, COM_BINLOG_DUMP, buf, logname_len + 10, 1))
  {
    error("Got fatal error sending the log dump command.");
    MYLOG(ZPC_ERROR, "Got fatal error sending the log dump command.");
    DBUG_RETURN(ERROR_STOP);
  }
        ...
        (type != LOAD_EVENT && type != CREATE_FILE_EVENT))
    {
      ...
      */
      ...
      if (retval != OK_CONTINUE){
			MYLOG(ZPC_NOTICE, "! OK CONTINUE retval=%d", retval);
			DBUG_RETURN(retval);
	  }
    }
    else
...
void* TRIGGER_process(void *args) {
	...
	typedef int (*DFP)();
	typedef int (*HFP)(ROW_DATA*);
	int re;
	char buf[1024];

	void *hdl=dlopen(server.mytrigger_so_file, RTLD_NOW | RTLD_GLOBAL);
	if(!hdl) {
		MYLOG(ZPC_ERROR, "dlopen %s faild[%s]",
				server.mytrigger_so_file, dlerror());
		exit(1);
	}
	IFP init_p = (IFP)dlsym(hdl, "init_proc");
	if (!init_p) {
		MYLOG(ZPC_ERROR, "dlsym init_proc faild");
		exit(1);
	}
	DFP deinit_p = (DFP)dlsym(hdl, "deinit_proc");
	if (!deinit_p) {
		MYLOG(ZPC_ERROR, "dlsym deinit_proc faild");
		exit(1);
	}
	HFP process = (HFP)dlsym(hdl, "process");
	if (!process) {
		MYLOG(ZPC_ERROR, "dlsym process faild");
		exit(1);
	}

	MYLOG(ZPC_DEBUG, "python script %s", server.python_script);
	if (init_p(server.python_script) < 0) {
		MYLOG(ZPC_ERROR, "init_proc faild.");
		exit(1);
	}
	for (;;) {
		/* get data from queue */
		...
		re = process(td);
		MYLOG(ZPC_DEBUG, "process return=%d", re);
		if (re < 0) {
			MYLOG(ZPC_ERROR, "process faild");
			exit(1);
		}

		/* record mytrigger info */
		if (re == RECORED_POSITION_F) {
			...
			re = snprintf(buf, 
				...
				pass,
				port,
				(uintmax_t)server.self_server_id,
				td->stmt_index);
			if (re >= 1024) {
				MYLOG(ZPC_ERROR, "info file buf was truncated");
				exit(1);
			}
			if (pwrite(fileno(master_info_file_p), buf, re,0) < 0) {
				MYLOG(ZPC_ERROR, "pwrite info file faild");
				exit(1);
			}
			MYLOG(ZPC_DEBUG, "record position %s", buf);
		}
		...
		/* safe exit */
		if (shutdown_flag) {
			MYLOG(ZPC_NOTICE, "mytrigger-worker normal exit");
			exit(1);
		}
		/* safe restart */
		if (main_shutdown_flag) {
			MYLOG(ZPC_NOTICE, "mytrigger-worker restart");
			exit(2);
		}

	}
	if (deinit_p() < 0) {
		MYLOG(ZPC_ERROR, "deinit_proc faild.");
		exit(1);
	}
	dlclose(hdl);
}
...
int main(int argc, char** argv)
{
  ...
  {
    //usage();
	printf("usage: mytrigger [option] remote_logname\n");
	printf("example: mytrigger --defaults-file=mytrigger.cnf remote_logname\n");
	 ...
	 */
	/* log level */
	if (!strcasecmp(server.log_level, "debug"))
		server.verbosity = ZPC_DEBUG;
	else if (!strcasecmp(server.log_level, "verbose"))
		server.verbosity = ZPC_VERBOSE;
	else if (!strcasecmp(server.log_level, "notice"))
		server.verbosity = ZPC_NOTICE;
	else if (!strcasecmp(server.log_level, "warning"))
		server.verbosity = ZPC_WARNING;
	else if (!strcasecmp(server.log_level, "error"))
		server.verbosity = ZPC_ERROR;
	else {
		MYLOG(ZPC_ERROR, "No level %s, default verbose is used", server.log_level);
		server.verbosity = ZPC_VERBOSE;
	}
	...
	char buf[1024] = {0};
	memset(current_log_name, 0, _POSIX_PATH_MAX + 1);
	if (access(server.master_info_file, F_OK)) {
		printf("create mytrigger info file[%s]\n", server.master_info_file);
		MYLOG(ZPC_NOTICE, "create mytrigger info file[%s]\n", server.master_info_file);
		master_info_file_p = fopen(server.master_info_file, "w+");
		if (NULL == master_info_file_p) {
			error(strerror(errno));
			exit(1);
		}
		fprintf(master_info_file_p, "%s\n", *argv);
		strlcpy(current_log_name, *argv++, _POSIX_PATH_MAX + 1);
		fprintf(master_info_file_p, "%ju\n", (uintmax_t)start_position);
		fprintf(master_info_file_p, "%s\n", host);
		fprintf(master_info_file_p, "%s\n", user);
		fprintf(master_info_file_p, "%s\n", pass);
		fprintf(master_info_file_p, "%d\n", port);
		fprintf(master_info_file_p, "%lu\n", server.self_server_id);
		fflush(master_info_file_p);
	} else { // file exists
		printf("read mytrigger info file[%s]\n", server.master_info_file);
		MYLOG(ZPC_NOTICE, "read mytrigger info file[%s]\n", server.master_info_file);
		master_info_file_p = fopen(server.master_info_file, "r+");
		if (NULL == master_info_file_p) {
			error(strerror(errno));
		}
		fgets(buf, sizeof(buf), master_info_file_p);
		buf[strlen(buf) - 1] = '\0';
		//printf("log name: %s\n", buf);
		...
		strlcpy(server.master_host, buf, sizeof(server.master_host));
		host = server.master_host;
		fgets(buf, sizeof(buf), master_info_file_p);
		buf[strlen(buf) - 1] = '\0';
		//printf("master user: %s\n", buf);
		strlcpy(server.master_user, buf, sizeof(server.master_user));
		user = server.master_user;
		fgets(buf, sizeof(buf), master_info_file_p);
		buf[strlen(buf) - 1] = '\0';
		//printf("master pass: %s\n", buf);
		strlcpy(server.master_pass, buf, sizeof(server.master_pass));
		pass = server.master_pass;
		fgets(buf, sizeof(buf), master_info_file_p);
		buf[strlen(buf) - 1] = '\0';
		...
		server.self_server_id = (int)strtoumax(buf, NULL, 10);
		//printf("\n");
	}
	/* zpc add */
	printf("log name: %s\n", current_log_name);
	...
	MYLOG(ZPC_NOTICE, "log name: %s", current_log_name);
	MYLOG(ZPC_NOTICE, "position: %ju", (uintmax_t)start_position);
	MYLOG(ZPC_NOTICE, "master host: %s", host);
	MYLOG(ZPC_NOTICE, "master user: %s", user);
	MYLOG(ZPC_NOTICE, "master pass: %s", pass);
	MYLOG(ZPC_NOTICE, "master port: %ju", (uintmax_t)port);
	MYLOG(ZPC_NOTICE, "self server id: %ju", (uintmax_t)server.self_server_id);
	...
	if (0 != err) {
		error("sql thread create faild, main thread exit.");
		MYLOG(ZPC_ERROR, "sql thread create faild, main thread exit.");
		exit(1);
	}
