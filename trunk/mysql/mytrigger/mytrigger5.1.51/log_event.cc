...
log_event_print_value(IO_CACHE *file, const uchar *ptr,
                      uint type, uint meta,
                      char *typestr, size_t typestr_length,MY_DATA* list,int& i)
{
  uint32 length= 0;
...
beg:
  ...
  case MYSQL_TYPE_TINY:
    {
	  ...
	  list[i].data=new int((int) (signed char) *ptr);
	  //memcpy(list[i].data, ptr, 1);
	  list[i].type=type;
	  list[i].length=1;
	  i++;
      return 1;
  ...
  case MYSQL_TYPE_NEWDECIMAL:
    {
      ...
      my_snprintf(typestr, typestr_length, "DECIMAL(%d,%d)",
                  precision, decimals);
	  list[i].data=new char[512];
	  memcpy(list[i].data,buff,bin_size);
	  list[i].type=type;
	  list[i].length=bin_size;
	  i++;
      return bin_size;
    }
  ...
  case MYSQL_TYPE_DATETIME:
    {
	  ...
	  list[i].data=new char[64];
	  sprintf((char*)list[i].data, "%04d-%02d-%02d %02d:%02d:%02d",
                  d / 10000, (d % 10000) / 100, d % 100,
                  t / 10000, (t % 10000) / 100, t % 100);
	  list[i].type=type;
	  list[i].length=8;
  ...
  case MYSQL_TYPE_TIME:
    {
	  ...
	  list[i].data=new char[32];
	  sprintf((char*)list[i].data, "'%02d:%02d:%02d'",
                  i32 / 10000, (i32 % 10000) / 100, i32 % 100);
	  list[i].type=type;
	  list[i].length=3;
	  i++;
  ...
  case MYSQL_TYPE_DATE:
    {
	  ...
	  list[i].data=new char[32];
	  sprintf((char*)list[i].data, "'%04d:%02d:%02d'",
                  (i32 / (16L * 32L)), (i32 / 32L % 16L), (i32 % 32L));
	  list[i].type=type;
	  list[i].length=3;
	  i++;
  ...
  case MYSQL_TYPE_YEAR:
    {
	  ...
	  list[i].data=new char[64];
	  sprintf((char*)list[i].data, "%04d", i32+ 1900);
	  list[i].type=type;
	  list[i].length=1;
	  i++;
      return 1;
  ...
  case MYSQL_TYPE_ENUM:
    ...
    case 1:
	  ...
	  list[i].data=new char[64];
	  sprintf((char*)list[i].data, "%d", (int) *ptr);
	  list[i].type=type;
	  list[i].length=1;
	  i++;
      return 1;
    ...
    default:
	  ...
	  list[i].data=new char[64];
	  sprintf((char*)list[i].data, "!! Unknown ENUM packlen=%d", length);
	  list[i].type=type;
	  list[i].length=0;
	  i++;
      return 0;
  ...
  case MYSQL_TYPE_BLOB:
    ...
    case 1:
	  ...
	  list[i].data=new char[length];
	  memcpy((char*)list[i].data,ptr+1,length);
	  list[i].type=type;
	  list[i].length=length + 1;
	  i++;
      return length + 1;
    case 2:
      length= uint2korr(ptr);
      ...
      my_snprintf(typestr, typestr_length, "BLOB/TEXT");
	   list[i].data=new char[length];
	  memcpy((char*)list[i].data,ptr+2,length);
	  list[i].type=type;
	  list[i].length=length + 2;
	  i++;
      return length + 2;
    case 3:
	  ...
	  list[i].data=new char[length];
	  memcpy((char*)list[i].data,ptr+3,length);
	  list[i].type=type;
	  list[i].length=length + 3;
	  i++;
      return length + 3;
    case 4:
	  ...
	  list[i].data=new char[length];
	  memcpy((char*)list[i].data,ptr+4,length);
	  list[i].type=type;
	  list[i].length=length + 4;
	  i++;
      return length + 4;
    default:
      my_b_printf(file, "!! Unknown BLOB packlen=%d", length);
	  list[i].data=new char[32];
	  sprintf((char*)list[i].data, "!! Unknown BLOB packlen=%d", length);
	  list[i].type=type;
	  list[i].length=0;
	  i++;
      return 0;
  ...
  case MYSQL_TYPE_VAR_STRING:
	...
	if(my_length<256)
		{
		my_length= *ptr;
		list[i].data=new char[my_length+1];
		memcpy((char*)list[i].data,ptr+1,my_length);
		memset((char*)list[i].data+my_length,'\0',1);
		list[i].type=type;
	  	list[i].length=my_length;
		}
	else
		{
		my_length= uint2korr(ptr);
		list[i].data=new char[my_length+1];
		memcpy((char*)list[i].data,ptr+2,my_length);
		memset((char*)list[i].data+my_length,'\0',1);
		list[i].type=type;
	  	list[i].length=my_length;
		}
  ...
  case MYSQL_TYPE_STRING:
	...
	if(my_length2<256)
		{
		my_length2= *ptr;
		list[i].data=new char[my_length2+1];
		memcpy((char*)list[i].data,ptr+1,my_length2);
		memset((char*)list[i].data+my_length2,'\0',1);
		list[i].type=type;
	  	list[i].length=my_length2;
		}
	else
		{
		my_length2= uint2korr(ptr);
		list[i].data=new char[my_length2+1];
		memcpy((char*)list[i].data,ptr+2,my_length2);
		memset((char*)list[i].data+my_length2,'\0',1);
		list[i].type=type;
	  	list[i].length=my_length2;
		}
  ...
  default:
    {
      ...
      my_b_printf(file,
                  "!! Don't know how to handle column type=%d meta=%d (%s)",
                  type, meta, tmp);
	  list[i].data=new char[128];
	  	sprintf((char*)list[i].data,"!! Don't know how to handle column type=%d meta=%d (%s)",
                  type, meta, tmp);
		list[i].type=type;
	  	list[i].length=0;
		i++;
...
Rows_log_event::print_verbose_one_row(IO_CACHE *file, table_def *td,
                                      ...
                                      const uchar *value, const uchar *prefix,MY_DATA* one_row_list)
{
  ...
  for (size_t i= 0; i < td->size(); i ++)
  {
    ...
    if (is_null)
    {
	  ...
	  one_row_list[k].data=new char[8];
	  sprintf((char*)one_row_list[k].data,"NULL\0");
	  one_row_list[k].length=0;
	  one_row_list[k].type=td->type(i);
	  k++;
    }
    else
    {
                                   ...
                                   PRINT_EVENT_INFO *print_event_info)
{
  ...
  //------------------------END INIT---------------------------------
    ...
    if (!(length= print_verbose_one_row(file, td, print_event_info,
                                  &m_cols, value,
                                  (const uchar*) sql_clause1,my_row_list)))
      goto end;
    value+= length;
    ...
    if (sql_clause2)
    {
      if (!(length= print_verbose_one_row(file, td, print_event_info,
                                      &m_cols_ai, value,
                                      (const uchar*) sql_clause2,my_row_list_update)))
        goto end;
      value+= length;
    }
  //----------To Use my_row_list-----------------------
...
#endif
void Log_event::DeleteTriggerData(TRIGGER_DATA* mytriggerdata)
{
	//-------delete my_row_list-----------------
    for(int   n   =   0;   n   <   mytriggerdata->filednum;   n++) {
		if(mytriggerdata->row_list[n].data!=NULL){
        delete   mytriggerdata->row_list[n].data; }
    	}
	delete[]   mytriggerdata->row_list; 
  	//----------------------------------------
  	//-------delete my_row_list_update-----------------
  	if (mytriggerdata->ioperate_type==24)
	  	{
	  		for(int   m   =   0;   m   <   mytriggerdata->filednum;   m++){ 
				if(mytriggerdata->row_list_update[m].data!=NULL){
	        	delete   mytriggerdata->row_list_update[m].data;} 
	  			}
			delete[]   mytriggerdata->row_list_update; 
	  	}
  	//----------------------------------------
