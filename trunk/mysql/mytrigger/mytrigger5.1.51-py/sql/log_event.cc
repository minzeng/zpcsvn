...
log_event_print_value(IO_CACHE *file, const uchar *ptr,
                      uint type, uint meta,
                      char *typestr, size_t typestr_length,FIELD_DATA* list,int& i)
{
  uint32 length= 0;
...
beg:
  ...
  case MYSQL_TYPE_LONG:
    {
	  ...
	  list[i].d = new int32(si);
	  //list[i].d = calloc(1, sizeof(int32));
	  //*(int32*)list[i].d = si;
	  list[i].d_type = type;
	  list[i].d_len = 4;
	  i++;
  ...
  case MYSQL_TYPE_TINY:
    {
	  ...
	  list[i].d = new int((int) (signed char) *ptr);
	  //list[i].d = calloc(1, sizeof(signed char));
	  //*(signed char*)list[i].d = (signed char) *ptr;
	  list[i].d_type = type;
	  list[i].d_len = 1;
	  i++;
      return 1;
  ...
  case MYSQL_TYPE_LONGLONG:
    {
      ...
      if (si < 0)
      {
        ...
        my_b_printf(file, " (%s)", tmp);
		//list[i].data=new ulonglong(ui);
      }
      my_snprintf(typestr, typestr_length, "LONGINT");
	  list[i].d = new longlong(si);
	  list[i].d_type = type;
	  list[i].d_len = 8;
	  i++;
      return 8;
  ...
  case MYSQL_TYPE_NEWDECIMAL:
    {
      ...
      my_snprintf(typestr, typestr_length, "DECIMAL(%d,%d)",
                  precision, decimals);
	  list[i].d = new char[strlen(buff) + 1];
	  memcpy(list[i].d, buff, strlen(buff) + 1);
	  list[i].d_type = type;
	  list[i].d_len = strlen(buff);
	  i++;
      return bin_size;
    }
  ...
  case MYSQL_TYPE_DATETIME:
    {
	  ...
	  list[i].d=new char[64];
	  sprintf((char*)list[i].d, "%04d-%02d-%02d %02d:%02d:%02d",
                  (int)d / 10000, (int)(d % 10000) / 100, (int)d % 100,
                  (int)t / 10000, (int)(t % 10000) / 100, (int)t % 100);
	  list[i].d_type = type;
	  list[i].d_len = strlen((char*)list[i].d);
	  i++;
      return 8;
    }
  ...
  case MYSQL_TYPE_TIME:
    {
	  ...
	  list[i].d = new char[32];
	  sprintf((char*)list[i].d, "'%02d:%02d:%02d'",
                  i32 / 10000, (i32 % 10000) / 100, i32 % 100);
	  list[i].d_type = type;
	  list[i].d_len = strlen((char*)list[i].d);
	  i++;
  ...
  case MYSQL_TYPE_DATE:
    {
	  ...
	  list[i].d=new char[32];
	  sprintf((char*)list[i].d, "'%04d:%02d:%02d'",
  ...
  case MYSQL_TYPE_YEAR:
    {
	  ...
	  list[i].d=new char[64];
	  sprintf((char*)list[i].d, "%04d", i32+ 1900);
	  list[i].d_type = type;
	  list[i].d_len = strlen((char*)list[i].d);
	  i++;
      return 1;
  ...
  case MYSQL_TYPE_ENUM:
    ...
    case 1:
	  ...
	  list[i].d=new char[64];
	  sprintf((char*)list[i].d, "%d", (int) *ptr);
	  list[i].d_type = type;
	  list[i].d_len = strlen((char*)list[i].d);
	  i++;
      return 1;
    ...
    default:
	  ...
	  list[i].d=new char[64];
	  sprintf((char*)list[i].d, "!! Unknown ENUM packlen=%d", length);
	  list[i].d_type=type;
	  list[i].d_len=strlen((char*)list[i].d);
	  i++;
      return 0;
  ...
  case MYSQL_TYPE_BLOB:
    ...
    case 1:
	  ...
	  list[i].d=new char[length];
	  memcpy((char*)list[i].d,ptr+1,length);
	  list[i].d_type=type;
	  list[i].d_len=length + 1;
	  i++;
      return length + 1;
    case 2:
      length= uint2korr(ptr);
      ...
      my_snprintf(typestr, typestr_length, "BLOB/TEXT");
	   list[i].d=new char[length];
	  memcpy((char*)list[i].d,ptr+2,length);
	  list[i].d_type=type;
	  list[i].d_len=length + 2;
	  i++;
      return length + 2;
    case 3:
	  ...
	  list[i].d=new char[length];
	  memcpy((char*)list[i].d,ptr+3,length);
	  list[i].d_type=type;
	  list[i].d_len=length + 3;
	  i++;
      return length + 3;
    case 4:
	  ...
	  list[i].d=new char[length];
	  memcpy((char*)list[i].d,ptr+4,length);
	  list[i].d_type=type;
	  list[i].d_len=length + 4;
	  i++;
      return length + 4;
    default:
      my_b_printf(file, "!! Unknown BLOB packlen=%d", length);
	  list[i].d=new char[32];
	  sprintf((char*)list[i].d, "!! Unknown BLOB packlen=%d", length);
	  list[i].d_type=type;
	  list[i].d_len=strlen((char*)list[i].d);
	  i++;
      return 0;
  ...
  case MYSQL_TYPE_VAR_STRING:
	...
	if(my_length<256)
		{
		my_length= *ptr;
		list[i].d=new char[my_length+1];
		memset((char*)list[i].d, 0, my_length+1);
		memcpy((char*)list[i].d, ptr+1, my_length);
		list[i].d_type=type;
	  	list[i].d_len=my_length;
		}
	else
		{
		my_length= uint2korr(ptr);
		list[i].d=new char[my_length+1];
		memset((char*)list[i].d, 0, my_length+1);
		memcpy((char*)list[i].d,ptr+2,my_length);
		list[i].d_type=type;
	  	list[i].d_len=my_length;
		}
	i++;
  ...
  case MYSQL_TYPE_STRING:
	...
	if(my_length2<256)
		{
		my_length2= *ptr;
		list[i].d=new char[my_length2+1];
		memset((char*)list[i].d, 0, my_length+1);
		memcpy((char*)list[i].d,ptr+1,my_length2);
		list[i].d_type=type;
	  	list[i].d_len=my_length2;
		}
	else
		{
		my_length2= uint2korr(ptr);
		list[i].d=new char[my_length2+1];
		memset((char*)list[i].d, 0, my_length+1);
		memcpy((char*)list[i].d,ptr+2,my_length2);
		list[i].d_type=type;
	  	list[i].d_len=my_length2;
		}
	i++;
  ...
  default:
    {
      ...
      my_b_printf(file,
                  "!! Don't know how to handle column type=%d meta=%d (%s)",
                  type, meta, tmp);
	  list[i].d=new char[128];
	  	sprintf((char*)list[i].d,"!! Don't know how to handle column type=%d meta=%d (%s)",
                  type, meta, tmp);
		list[i].d_type=type;
	  	list[i].d_len=0;
		i++;
										...
										FIELD_DATA* one_row_list)
{
  ...
  for (size_t i= 0; i < td->size(); i ++)
  {
    ...
    if (is_null)
    {
	  ...
	  one_row_list[k].d = calloc(8, sizeof(char));
	  sprintf((char*)one_row_list[k].d, "NULL");
	  one_row_list[k].d_len = 0;
	  one_row_list[k].d_type = MYSQL_TYPE_NULL;
	  k++;
    }
    else
    {
      ...
      size_t size= log_event_print_value(file, value,
                                         ...
                                         typestr, sizeof(typestr),one_row_list,k);
      if (!size)
        return 0;
                                   ...
                                   PRINT_EVENT_INFO *print_event_info)
{
      ...
      !(td= map->create_table_def()))
  {
	...
	ROW_DATA* rd; //mytriggerdata;
	FIELD_DATA* field_list_new;
	FIELD_DATA* field_list_old;
	for (const uchar *value= m_rows_buf; value < m_rows_end; ) {
		rd = (ROW_DATA*)calloc(1, sizeof(ROW_DATA));
		rd->log_pos = start_position;
		rd->stmt_index = ++stmt_index;
		field_list_new = (FIELD_DATA*)calloc(m_width, sizeof(FIELD_DATA));
		if (sql_clause2) {
			field_list_old = (FIELD_DATA*)calloc(m_width, sizeof(FIELD_DATA));
		}
		size_t length;
		my_b_printf(file, "### %s %s.%s\n",
			sql_command, map->get_db_name(), map->get_table_name());
		/* Print the first image */
		if (!(length= print_verbose_one_row(file, td, print_event_info,
			&m_cols, value, (const uchar*) sql_clause1, field_list_new))) {
			goto end;
		}
		value+= length;
		...
		if (sql_clause2) {
			if (!(length= print_verbose_one_row(file, td, print_event_info,
				&m_cols_ai, value, 
				(const uchar*) sql_clause2, field_list_old))) {
				goto end;
			}
			value+= length;
		}
		if (type_code == UPDATE_ROWS_EVENT) {
			rd->field_list_new = field_list_old;
			rd->field_list_old = field_list_new;
		} else {
			rd->field_list_new = field_list_new;
		}
		rd->log_pos = log_pos;
		rd->event_type = type_code;
		rd->field_list_len = (int)m_width;
		strlcpy(rd->db_name, map->get_db_name(), MAX_NAME_LEN);
		strlcpy(rd->tb_name, map->get_table_name(), MAX_NAME_LEN);
		strlcpy(rd->log_name, current_log_name, MAX_NAME_LEN);

		if(get_flags(STMT_END_F) && value >= m_rows_end) {
			rd->stmt_end_f = 1;
		}

		if (stmt_index > stmt_index_start) {
			enqueue(rd);
			stmt_index_start = 0;
		} else {
			Log_event::DeleteTriggerData(rd);
		}

		if(get_flags(STMT_END_F) && value >= m_rows_end) {
			start_position = log_pos;
			stmt_index_start = 0;
			stmt_index = 0;
		}
	}
...
#endif
void Log_event::DeleteTriggerData(ROW_DATA* mytriggerdata)
{
	for(int n = 0; n < mytriggerdata->field_list_len; n++) {
		if(mytriggerdata->field_list_new[n].d != NULL) {
			delete mytriggerdata->field_list_new[n].d;
		}
	}
	free(mytriggerdata->field_list_new);
	if (mytriggerdata->event_type == UPDATE_ROWS_EVENT) {
		for(int m = 0; m < mytriggerdata->field_list_len; m++) {
			if(mytriggerdata->field_list_old[m].d != NULL) {
				delete mytriggerdata->field_list_old[m].d;
			}
		}
		free(mytriggerdata->field_list_old);
	}
	free(mytriggerdata);
}
...
int enqueue(void *e){
	...
	while (qs >= server.row_queue_size) { /* avoid alloc huge memory*/
		pthread_cond_wait(&qready, &q_lock);	
	}
	item = (struct EVENT_ITEM *)calloc(1, sizeof(struct EVENT_ITEM));
	if (NULL == item) {
		MYLOG(ZPC_ERROR, "calloc struct EVENT_ITEM faild in enqueue");
		return -1;
	}
	item->e = e;
...
void
maketimeout(struct timespec *tsp, long second) {
	struct timeval now;
...
void *dequeue() {
	...
	while (TAILQ_EMPTY(&event_q_head)) {
		maketimeout(&ts, 3);
		re = pthread_cond_timedwait(&qready, &q_lock, &ts);
		if (re == ETIMEDOUT) {
			if (shutdown_flag) {
				MYLOG(ZPC_ERROR, "mytrigger-worker normal exit");
				exit(1);
			} else if (main_shutdown_flag) {
				MYLOG(ZPC_ERROR, "mytrigger-worker restart");
				exit(2);	
			} else {
				continue;	
			}
...
void zpc_log(int level, const char *fmt, ...) {
	...
	fp = (server.log_file == NULL) ? stdout : fopen(server.log_file,"a");
	if (!fp) return;
